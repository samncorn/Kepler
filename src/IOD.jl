...
function herget_kernel(rho12, observations, obs1, obs2, gm, c)
    ...
    return Iterators.map(o -> herget_residuals_with_partials(o, orbit, dx1_dp1, dv1_dp2), observations)
end

function herget_residuals_with_partials(obs, orbit, dx1_dp1, dv1_dp2)
    resid, J_x, J_v = compute_residuals_with_partials(obs, orbit)
...
function herget_solve_with_partials(obs1, obs2, rho1, rho2, gm, c)
    ...
    vel1, vel2 = lambert_direct(pos1, pos2, t2 - t1, gm, n_revs)
